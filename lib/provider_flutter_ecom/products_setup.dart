...
class Products with ChangeNotifier{
  //List<Product> _products = [
    /*Product(
        ...
        quantity: 65,
        isPopular: false, isFavorite: false),
    Product(
    ...
    Product(
        ...
        productCategoryName: 'Phones',
        quantity: 6423,
        isPopular: true, isFavorite: false),
    Product(
        id: 'Huawei P40 Pro',
  ...
  List<Product> findbyCatagory(String CatagoryName){
    List _catagorylist=_products.where((element) => element.productCategoryName.toLowerCase().contains(CatagoryName.toLowerCase())).toList();
    return [..._catagorylist];
  }
  List<Product> get popularProducts_fc{
  ...
  List<Product> searchQuery(String value) {
    List _searchList = _products.where((element) => element.title.toLowerCase().contains(value.toLowerCase())).toList();

    if([..._searchList].length==0){
      _searchList = _products.where((element) => element.brand.toLowerCase().contains(value.toLowerCase())).toList();
      if([..._searchList].length==0){
        _searchList = _products.where((element) => element.productCategoryName.toLowerCase().contains(value.toLowerCase())).toList();
        return [..._searchList];
      }
      else{
        return [..._searchList];
